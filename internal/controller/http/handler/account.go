...
func (h *AccountHandler) Rename(c *gin.Context) {
	...
	if err != nil {
		if errors.Is(err, usecase.ErrNewUsernameRequired) {
			c.JSON(apierror.ClassifyHTTPStatus(err), gin.H{"error": err.Error()})
			return
		}
...
func (h *AccountHandler) FindByID(c *gin.Context) {
	var req entity.FindByIDRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(apierror.ClassifyHTTPStatus(err), gin.H{"error": err.Error()})
		return
	}
...
func (h *AccountHandler) FindByUsername(c *gin.Context) {
	var req entity.FindByUsernameRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(apierror.ClassifyHTTPStatus(err), gin.H{"error": err.Error()})
		return
	}
...
func (h *AccountHandler) Login(c *gin.Context) {
	...
	c.JSON(200, entity.LoginResponse{Token: accessToken, RefreshToken: refreshToken, AccountID: account.ID, Username: account.Username})
}
...
func (h *AccountHandler) UploadAvatar(c *gin.Context) {
	...
	avatarURL, err := h.accountService.UploadAvatar(
		c.Request.Context(),
		accountID,
		objectKey,
...
func (h *AccountHandler) Refresh(c *gin.Context) {
	...
	c.JSON(http.StatusOK, entity.LoginResponse{Token: newToken, AccountID: accountID, Username: username})
}
