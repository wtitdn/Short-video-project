...
func (f *FeedService) GetVideoByIDs(ctx context.Context, videoIDs []uint) ([]*entity.Video, error) {
	...
	videoMap := make(map[uint]*entity.Video)
	//L1:本地缓存
	var missedL1 []uint
...
// 查询最新视频 (冷热分离 + 游标分页)
func (f *FeedService) ListLatest(ctx context.Context, limit int, latestBefore time.Time, viewerAccountID uint) (entity.ListLatestResponse, error) {
	// 获取 ZSET 中最老的一条数据
	zsetTail, err := f.rediscache.ZRangeWithScores(ctx, f.rediscache.Key("feed:global_timeline"), 0, 0)

	if err != nil {
		return entity.ListLatestResponse{}, err
	}
	...
	if isZsetEmpty {
		...
		if v == "EMPTY_DB" {
			return entity.ListLatestResponse{HasMore: false}, nil
		}
	...
	return entity.ListLatestResponse{
...
// 按照点赞数查询视频
func (f *FeedService) ListLikesCount(ctx context.Context, limit int, cursor *entity.LikesCountCursor, viewerAccountID uint) (entity.ListLikesCountResponse, error) {
	videos, err := f.repo.ListLikesCountWithCursor(ctx, limit, cursor)
	if err != nil {
		return entity.ListLikesCountResponse{}, err
	}
	...
	resp := entity.ListLikesCountResponse{
		VideoList: feedVideos,
		HasMore:   hasMore,
	}
	if len(videos) > 0 {
...
// 按照关注列表查询视频
func (f *FeedService) ListByFollowing(ctx context.Context, limit int, latestBefore time.Time, viewerAccountID uint) (entity.ListByFollowingResponse, error) {
	doListByFollowingFromDB := func() (entity.ListByFollowingResponse, error) {
		videos, err := f.repo.ListByFollowing(ctx, limit, viewerAccountID, latestBefore)
		if err != nil {
			return entity.ListByFollowingResponse{}, err
		}
		var nextTime int64
		if len(videos) > 0 {
		...
		resp := entity.ListByFollowingResponse{
			VideoList: feedVideos,
			NextTime:  nextTime,
			HasMore:   hasMore,
		}
...
func (f *FeedService) ListByPopularity(ctx context.Context, limit int, reqAsOf int64, offset int, viewerAccountID uint, latestPopularity int64, latestBefore time.Time, latestIDBefore uint) (entity.ListByPopularityResponse, error) {
	...
	if f.rediscache != nil {
		...
		if err == nil && len(members) == 0 {
			if offset > 0 {
				return entity.ListByPopularityResponse{
					VideoList:  []entity.FeedVideoItem{},
					AsOf:       asOf.Unix(),
					NextOffset: offset,
					HasMore:    false,
				}, nil
		...
		if err == nil && len(members) > 0 {
			...
			if err == nil {
				byID := make(map[uint]*entity.Video, len(videos))
				for _, v := range videos {
					byID[v.ID] = v
				}
				ordered := make([]*entity.Video, 0, len(ids))
				for _, id := range ids {
					if v := byID[id]; v != nil {
				...
				if err != nil {
					return entity.ListByPopularityResponse{}, err
				}
				resp := entity.ListByPopularityResponse{
					VideoList:  items,
					AsOf:       asOf.Unix(),
...
func (f *FeedService) buildFeedVideos(ctx context.Context, videos []*entity.Video, viewerAccountID uint) ([]entity.FeedVideoItem, error) {
	feedVideos := make([]entity.FeedVideoItem, 0, len(videos))
...
func buildOrderedResult(orderedIDs []uint, dataMap map[uint]*entity.Video) []*entity.Video {
	res := make([]*entity.Video, 0, len(orderedIDs))
...
func (f *FeedService) ListByTag(ctx context.Context, tagName string, limit int, viewerAccountID uint) ([]entity.FeedVideoItem, error) {
